...
def view_category(request, danh_muc_id):
    ...
    products = base_products.prefetch_related('hinhanhs')
    ...
    if selected_brands:
        products = products.filter(ThuongHieu__in=selected_brands)

    # C. Lọc theo Giá
    price_range = request.GET.get('price')
    if price_range:
        if price_range == 'under_100':
            products = products.filter(DonGia__lt=100000)
        elif price_range == '100_300':
            products = products.filter(DonGia__range=(100000, 300000))
        elif price_range == '300_500':
            products = products.filter(DonGia__range=(300000, 500000))
        elif price_range == 'above_500':
            products = products.filter(DonGia__gt=500000)
